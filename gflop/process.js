...
d3.csv('hpl-quanah-slurm-3-3-2021.csv').then(dataRaw=>{
    ...
    dataRaw.forEach(r=>{
        ...
        if (r.timeStartCut<timeRange[0])
            timeRange[0] = r.timeStartCut;
    ...
    d3.json(`https://influx.ttu.edu:8080/v1/metrics?start=${timeRange[0].toISOString()}&end=${timeRange[1].toISOString()}&interval=1m&value=max&compress=false`).then(_data=>{
        ...
        dataRaw.forEach(r=>{
            ...
            const item = {id:comp,class:r.class,arr:{}};
            const startIndex = Math.round(time2index(+r.timeStartCut));
            const endIndex = startIndex + timeCutCount;
        ...
        d3.range(0,timeCutCount).forEach(t=>{
