...
function makeComputeLegend(){
    ...
    const r=3;
    const g = d3.select('#legendComputeSize g');
    ...
    let offset = r*(d3.sum(ticks,d=>scale(d)));
    ticks.reverse();
    let t=0;
    ...
    gback.selectAll('circle').data(tickD)
        .join('circle').attr('r',r).attr('fill','white').attr('stroke','black').attr('stroke-width',0.2).attr('transform',d=>`translate(${d.o},0) scale(${d.s})`);
    gback.selectAll('text').data([tickD[0],tickD[ticks.length-1]])
        .join('text').text(d=>d.d).attr('text-anchor',(d,i)=>i?'end':'start').attr('transform',(d,i)=>`translate(${d.o+(i?(-5):5)},${5})`);
...
function makeUserLegend(){
    ...
    gback.selectAll('circle').data(ticks)
        .join('circle').attr('r',UserIcon.r).attr('fill','white').attr('stroke','black').attr('stroke-width',0.2).attr('transform',d=>`translate(0,${-UserIcon.r*userScale(d)+offset}) scale(${userScale(d)})`);
    gback.selectAll('text').data([ticks[0],ticks[ticks.length-1]])
        .join('text').text(d=>d).attr('transform',d=>`translate(${offset*1.5},${-UserIcon.r*userScale(d)*2+offset})`);

    g.selectAll('g.iconu').data([small]).join(enter=>{
        enter.append('g').attr('class','iconu').attr('transform',d=>`translate(0,${-UserIcon.r*userScale(d)+offset}) scale(${userScale(d)})`)
            .append('path').attr('d',UserIcon.d).attr('transform',d=>`translate(${UserIcon.offset},${UserIcon.offset}) scale(${UserIcon.scale})`);
        return enter;
    },update=>update.attr('transform',d=>`translate(0,${-UserIcon.r*userScale(d)+offset}) scale(${userScale(d)})`));
    g.selectAll('line').data([ticks[0],ticks[ticks.length-1]])
