    ...
    constructor(url) {
        ...
        if (!this.isRealTime) {
            let updatePromise=(_.isString(url)?d3.json(url):url).then((data) => {
                const node_list = Object.keys(data.nodes_info);
                if(!data.nodes_info[node_list[0]].cpu_cores){
                    node_list.forEach(comp=>{
                        data.nodes_info[comp].cpu_cores = data.nodes_info[comp].job_id.map(jids=>jids.map(j=>data.jobs_info[j].cpu_cores/data.jobs_info[j].node_list.length));
                    })
                }
