...
function _createdata({tree,computers,jobs,users,jobByNames,sampleS}){
    ...
    function updateLinkCU(c) {
        ...
        computers[c.name].job_id[0].forEach(d=>{
            ...
            if (!userD[jobs[d].user_name])
                userD[jobs[d].user_name] = 0;
            userD[jobs[d].user_name] = Math.max((+Layout.currentTime) - jobs[d].start_time,userD[jobs[d].user_name])
        });

        d3.entries(userL).forEach(u => dataIn.links.push({source: c.name, target: u.key, value: u.value, duration: userD[u.key]}));
    }
...
// Layout.jobShow
    ...
    Object.keys(users).forEach(u=>{
        ...
        dataIn.nodes.push({id:u,type:'user',data:users[u],value:u,
            duration: users[u].duration,
            r: 10,
            tooltip:u,drawData:[{
                invalid: undefined,
                scale:1,
                offset:UserIcon.offset,
               d:UserIcon.d}]})
    });
