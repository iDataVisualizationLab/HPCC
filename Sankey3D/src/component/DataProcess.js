...
import AreaStack from "./AreaStack_user"
import LineStack from "./LineStack_user"
...
import {getRefRange, getUrl} from "./ulti"
...
import {calculateCluster} from './cluster'
import {colorLegend} from "./leva/ColorLegend";
...
function DataProcss({onLoad, theme, _data}) {
    ...
    useLayoutEffect(() => {
        const {scheme, draw3DData, dimensions, layout, sankeyData} = handleData(_data);
    ...
    const [{selectedSer, selectedSer2}, setSelectedSer] = useControls("Setting", () => ({
        selectedSer: {
            options: optionsColor, label: "Metric", value: 0,
            onChange: (val) => {
    ...
    const [allplycluster] = useControls("DatasetCluster", () => ({
        DataProcssly: button((get) => {
            console.log('clicked DataProcssly')
            recalCluster(scheme, dimensions, ({clusterInfo, scheme}) => {
                if (selectedSer === 'cluster') {
                    setScheme(scheme);
                    updateColor(_draw3DData, scheme, undefined, clusterInfo);
    ...
    const showRadar = useControls("DatasetCluster", {
        Clusters: viz({
            value: 0,
            label: clusterInfo.clusterInfo && <> DatasetCluster
                inputdata: {clusterInfo.clusterInfo.input} ({d3.format(",.1%")(clusterInfo.clusterInfo.input / clusterInfo.clusterInfo.total)})<br/>
            ...
            com: <div style={{width: '100%', display: 'flex', flexWrap: "wrap", justifyContent: "space-between"}}>{
                clusterInfo && clusterInfo.cluster.map((c, i) => <div key={c.labels} style={{display: 'inline-block'}}>
                    <div style={{position: 'relative'}}
                        ...
                        <Radar width={100} height={100} data={[c.__metrics]} area={true} mean={true}
                               meanColor={"rgba(0,0,0,0.52)"}
                               fill={clusterInfo.colorCluster ? clusterInfo.colorCluster(c.name) : '#fff'}
                               stroke={'match'}
                        />
                        ...
                        <p style={{
                            ...
                            color: 'black',
                            fontSize: 12
                        }}>{c.arr.length}<br/>{i ? '' : ' temporal instances'}</p>
                    </div>
                </div>)
            }
                {
                    (clusterInfo && clusterInfo.cluster) ? <div style={{
                        ...
                        <div style={{display: 'flex', alignItems: 'center', padding: 3}}>
                            <div style={{
                                ...
                                height: 20,
                                borderRadius: 10,
                                backgroundColor: clusterInfo.colorCluster ? clusterInfo.colorCluster('outlier') : '#fff'
                            }}/>
                            Outlier
                        </div>
                        <div style={{display: 'flex', alignItems: 'center', padding: 3}}>
                            <div style={{
                                ...
                                height: 20,
                                borderRadius: 10,
                                backgroundColor: clusterInfo.colorCluster ? clusterInfo.colorCluster('missing') : '#fff'
                            }}/>
        ...
        "Outliers": viz({
            label: clusterInfo.outlyingBins && <>Outliers: {(clusterInfo.outlyingBins.pointObject) ? Object.keys(clusterInfo.outlyingBins.pointObject).length : 0} temporal
                instances</>, value: 0,
            com: <div style={{width: '100%', display: 'flex', flexWrap: "wrap", justifyContent: "space-between"}}>
                {
                    clusterInfo.outlyingBins && clusterInfo.outlyingBins.map((c, i) => <div key={c.labels}
                                                                                            style={{display: 'inline-block'}}>
                        <div style={{position: 'relative'}}>
                            <Radar width={100} height={100}
                                   data={c.arr.map(d => clusterInfo.outlyingBins.pointObject[d.key])}
                                   fill={'none'}
                                   stroke={clusterInfo.colorCluster ? clusterInfo.colorCluster('outlier') : '#fff'}
        ...
        "Missing Dimension": viz({
            label: clusterInfo.outlyingBins && <>Missing
                Dimension: {(clusterInfo.outlyingBins.missingData) ? Object.keys(clusterInfo.outlyingBins.missingData).length : 0} temporal
                instances</>, value: 0,
    ...
    const recalCluster = useCallback((_scheme = scheme, _dimensions = dimensions, callback = () => {
        ...
        setTimeout(() => {
            ...
            let {cluster, clusterDescription, colorCluster, clusterInfo} = calculateCluster({
                data: flat,
    ...
    const handleData = useCallback((_data) => {
            ...
            layout[k].forEach((row, ri) => {
                row.forEach((col, ci) => {
                    if (col) {
                        const item = {
                            text: col,
                            position: [(gconfig.padding + ci * wA + layout[k].position[0]) + gconfig.size[0] / 2, -(gconfig.padding + ri * hA + gconfig.size[1] / 2) + layout[k].position[1], 0 + layout[k].position[2]]
                        }
        ...
        return {scheme, draw3DData, dimensions,sankeyData, layout}
    }, [layout]);
    ...
    function handleDataComputeByUser_core({computers, jobs, timespan}) {
        ...
        for (let comp in computers) {
            ...
            computers[comp].job_id.forEach((jIDs, i) => {
                if (jIDs.length) {
                    let jobArr = jIDs.map(j => jobs[j]);
                    let username = d3.groups(jobArr, d => d.user_name).map(d => ({
                        key: d[0],
                        value: d3.sum(d[1], (e) => e.node_list_obj[comp] ?? 0)
                    }));
                    username.total = d3.sum(username, e => e.value);
                    username.jobs = [jIDs, jobArr];
                    item['' + timespan[i]] = username.sort((a, b) => d3.ascending(a.key, b.key));
    ...
    const [{showLineChart}, setShowLineChart] = useControls("Graphic", () => ({
        'showLineChart': {
            value: false,
            label: 'Line chart'
        }
        ...
        <Grid container style={{height: "100vh", width: '100wh', overflow: 'hidden'}}>
            <Container maxWidth="lg">
                <div style={{height: "100vh",width:'100wh',overflow:'hidden'}}>
                    <Layout3D sankeyData={sankeyData} time_stamp={scheme.time_stamp}
                              color
                        layout={layout} time_stamp={scheme.time_stamp} data={draw3DData} line3D={line3D} selectService={selectedSer} stackOption={config.stackOption} getKey={(d)=>d.data.key+' '+d.data.timestep}/>
                </div>
                <Grid container>
                    <Grid item xs={12}>
