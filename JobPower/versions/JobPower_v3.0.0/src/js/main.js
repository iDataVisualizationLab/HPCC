...
const TIME_RANGES = [
  ...
  { label: 'Current time', value: -2 , default: true },
];
...
//     d3.select('#realTimeRange').on('change', function () {
...
function startRealTimePolling() {
  ...
  function fetchAndUpdate() {
    ...
    request = new Simulation(fetchDataAndProcess(realTimeParams, true));
  }
  fetchAndUpdate();
  initdraw();
    initTimeElement();
...
function loadHistoricalData() {
  ...
  const historicalParams = buildHistoricalParams(start, end);
  request = new Simulation(fetchDataAndProcess(historicalParams, true));
  initdraw();
    initTimeElement();
}
...
//     request = new Simulation(fetch('http://narumuu.ttu.edu/api/h100/', {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify(realTimeParams)
//     }).then(res => res.json()).then(apiData => {
//         const timeMap = new Map();
//         const allTimes = Array.from(new Set(apiData.nodes.map(d => d.time))).sort((a, b) => a - b);
//         allTimes.forEach((t, i) => timeMap.set(t, i));
...
//     }));
// }
// // else if (command.mode === 'historical') {}
