...
.factory('Loaddata', function($timeout,Dataset,_, Config) {
    ...
    function loadFile(){
        ...
        var promiseQueue;
        debugger
        if ((db === 'csv'&& choice.category==='hpcc')) { //reload hostlist
    ...
    function readFilecsv(file,separate,object) {
        ...
        function loadcsv(data,customformat) {
            db = "csv";
            newdatatoFormat(data.queue_status?data.queue_status:data, separate,customformat,data.timestamp);
            ...
            selectedService = object.customSetting.initAxis?object.customSetting.initAxis:serviceLists[0].text;
            inithostResults();
            formatService(true);
        ...
        setTimeout(() => {
            ...
            //
            //         }).catch(function(e){
            //             M.toast({html: 'Invalid file'})
            //         })
            //     })
            // }else{
                dataInformation.size = object.size;
                loadcsv(object.values,object.customSetting)
            // }
        }, 0);
    }
    Loaddata.reset();
