...
//rose chart
    ...
    //update the outlines
    var sg = blobWrapperg.selectAll(".radarStroke").data(d=>d);
    sg.exit().remove();
    sg.enter().append('path').attr('class','radarStroke');

    var blobWrapperpath = blobWrapperg.selectAll(".radarStroke");
    if (cfg.bin) { // bin type
        ...
        var cp = blobWrapperg.selectAll('clipPath').data(d=>d);
        cp.exit().remove();
        cp.enter().append('clipPath').append('path');
        ...
        blobWrapper.selectAll('.radarStroke').data(d=>d).enter()
            ...
            .style("stroke", (d, i) => cfg.color(i,d));
        blobWrapper.selectAll('.radarLine').data(d=>d).enter()
        ...
        //update the outlines
        blobWrapperg.selectAll('.radarLine').transition().call(drawMeanLine);
        blobWrapperg.selectAll('.radarQuantile').transition().call(drawQuantileArea);
        blobWrapperpath.style("fill", "none").transition()
