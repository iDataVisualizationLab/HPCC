d3.TimeSpace = function () {
    ...
    function showMetrics_plotly(name,index) {
        ...
        if (shap[name]){
            ...
            schema= temp.map(d=>serviceFullList_raw[d.index])
        }
        else
            schema =serviceFullList_raw;
        const data_in = schema.map((s,si) => {
            ...
            sampleS_raw[name][serviceListattr[s.idroot]].forEach((e,ti) => {
    ...
    function handle_data(data){
        ...
        let maxstep = sampleS_raw.timespan.length - 1;
        scaleTime = d3.scaleTime().domain([sampleS_raw.timespan[0], sampleS_raw.timespan[maxstep]]).range([0, maxstep]);
